...
export default function SlideShow() {
  ...
  function handleBackArrow() {
    const totalLength = slideContainerRef.current.children[2].clientWidth * 4;
    
    if (slideContainerRef.current.scrollLeft === 0) {
      setHideLeft("none");
    }
    
    if (slideContainerRef.current.scrollLeft === slideContainerRef.current.children[0].clientWidth {
      setHideRight("");
    }
